...
use crate::graphql::modules::schema::{UserType, ProductType};
use std::env;
use lazy_static::lazy_static;
use common_utils::error::ServiceError;

pub const NEW_PRODUCT_USER_CACHE: &str = "newPRODUCTPostofUser";

lazy_static! {
    static ref PRODUCT_KEY_PREFIX: String = std::env::var("REDIS_KEY_PREFIX").expect("JWT Secret Key Error");
}
...
pub fn get_post_cache_key(id: &str) -> String { 
    format!("{}:{}", PRODUCT_KEY_PREFIX.as_str(), id)
}
...
impl ToRedisArgs for ProductType { 
